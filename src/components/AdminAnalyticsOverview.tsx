  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-end">
        <div>
          <h2 className="text-2xl font-bold text-slate-900 drop-shadow-sm">System Analytics</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-gradient-to-br from-slate-50 via-slate-100 to-slate-200 border border-slate-300 rounded-3xl p-6 relative overflow-hidden shadow-[inset_0_1px_0_rgba(255,255,255,0.8),0_4px_10px_rgba(0,0,0,0.05)] group">
          ...
          <div className="relative z-10">
            ...
            <div className="text-4xl font-black text-slate-900">{stats.totalRecords.toLocaleString()}</div>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-slate-50 via-slate-100 to-slate-200 border border-slate-300 rounded-3xl p-6 relative overflow-hidden shadow-[inset_0_1px_0_rgba(255,255,255,0.8),0_4px_10px_rgba(0,0,0,0.05)] group">
          <div className="absolute inset-0 bg-gradient-to-tr from-transparent via-white/40 to-transparent opacity-0 group-hover:opacity-100 group-hover:translate-x-full duration-1000 transition-all -skew-x-12 transform scale-150 z-0 pointer-events-none"></div>
          <div className="relative z-10">
            <div className="w-12 h-12 bg-white rounded-xl shadow-sm border border-slate-200 flex items-center justify-center mb-4 text-amber-500">
      ...
      {/* Breakdown Metrics */}
      <div className="bg-white/70 backdrop-blur-md border border-slate-200 rounded-2xl shadow-sm p-6 mt-6">
        <h3 className="text-sm font-bold text-slate-900 mb-6 uppercase tracking-widest">Entity Breakdown</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="p-4 rounded-xl border border-slate-100 bg-slate-50 flex items-center justify-between">
            ...
            <span className="text-lg font-bold text-slate-900">{stats.doctors.toLocaleString()}</span>
          </div>
          <div className="p-4 rounded-xl border border-slate-100 bg-slate-50 flex items-center justify-between">
            <span className="text-xs font-bold text-slate-500 uppercase">Hospitals</span>
            <span className="text-lg font-bold text-slate-900">{stats.hospitals.toLocaleString()}</span>
          </div>
          <div className="p-4 rounded-xl border border-slate-100 bg-slate-50 flex items-center justify-between">
            <span className="text-xs font-bold text-slate-500 uppercase">Pharmacies</span>
            <span className="text-lg font-bold text-slate-900">{stats.pharmacies.toLocaleString()}</span>
          </div>
          <div className="p-4 rounded-xl border border-slate-100 bg-slate-50 flex items-center justify-between">
            <span className="text-xs font-bold text-slate-500 uppercase">Other/Labs</span>
            <span className="text-lg font-bold text-slate-900">{(stats.totalRecords - stats.doctors - stats.hospitals - stats.pharmacies).toLocaleString()}</span>
          </div>
        </div>
      </div>
    </div>
