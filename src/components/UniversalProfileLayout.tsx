  ...
  return (
    <div className="min-h-screen bg-[#060B14] font-sans pb-20 selection:bg-cyan-500/30">
             ...
             <div className="flex-1 text-center md:text-left">
                  ...
                  {type === 'ambulance' && (
                    <>
                      <div className="flex items-center gap-2 bg-slate-800/50 backdrop-blur-md px-4 py-2 rounded-xl border border-slate-700/50">
                        ...
                        <div>
                          <span className="text-white font-bold block leading-none">{Array.isArray(profile.ambulanceType) ? profile.ambulanceType.join(", ") : profile.ambulanceType || "Emergency Transport"}</span>
               ...
               <div className="mt-8 flex flex-wrap justify-center md:justify-start gap-4">
                 <button 
                   ...
                   className={`px-6 py-3 rounded-xl font-bold uppercase tracking-widest text-sm transition-all shadow-xl flex items-center justify-center gap-2 ${
                 ...
                 {type === 'doctor' && verified && (
                   <button 
                     onClick={() => {
                       window.dispatchEvent(new CustomEvent('open-telemedicine-fab', { 
                         detail: { action: 'urgent', doctorId: profile.id, doctorName: profile.name } 
                       }));
                     }}
                     className="px-6 py-3 rounded-xl font-bold uppercase tracking-widest text-sm transition-all shadow-[0_10px_20px_rgba(239,68,68,0.3)] flex items-center justify-center gap-2 bg-gradient-to-r from-red-500 to-rose-600 text-white hover:from-red-400 hover:to-rose-500 hover:scale-105"
                   >
            ...
            {activeTab === 'departments' && (
              <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4">
                <div className="bg-slate-900/40 backdrop-blur-xl rounded-[32px] p-8 md:p-10 border border-slate-700/50 shadow-xl">
                  ...
                  {/* Department-Centric Rendering */}
                  {profile.departments?.length > 0 ? (
                    <div className="space-y-8 mb-8">
                      {profile.departments.map((dept: any, idx: number) => (
                        <div key={idx} className="flex flex-col bg-slate-800/50 border border-slate-700/50 p-6 md:p-8 rounded-3xl shadow-sm hover:border-cyan-500/30 transition-colors relative overflow-hidden">
                          {/* Top accent */}
                          ...
                          <div className="mb-6">
                            ...
                            {dept.head && (
                              <p className="text-sm font-bold text-emerald-400 mt-2 uppercase tracking-widest flex items-center gap-2">
                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
                                HOD: {dept.head}
                              </p>
                            )}
                            {dept.description && <p className="text-sm text-slate-400 mt-3 leading-relaxed max-w-3xl">{dept.description}</p>}
                          </div>

                          {/* Department Roster */}
                          {dept.roster && dept.roster.length > 0 ? (
                            <div className="mt-4 border-t border-slate-700/50 pt-6">
                              <h5 className="text-sm font-bold text-slate-500 uppercase tracking-widest mb-4">Department Doctors ({dept.roster.length})</h5>
                              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                                {dept.roster.map((docItem: any, docIdx: number) => {
                                  // Legacy handling
                                  if (typeof docItem === 'string') {
                                    return (
                                      <div key={docIdx} className="flex items-center justify-between bg-slate-900/50 border border-slate-700/50 p-4 rounded-2xl">
                                  ...
                                  if (docItem.type === 'verified' || !docItem.type) {
                                    return (
                                      <Link href={`/profile/doctor/${docItem.id}`} key={docIdx} className="flex flex-col justify-center bg-gradient-to-r from-slate-800/80 to-slate-800/40 border border-cyan-500/20 p-5 rounded-2xl group hover:border-cyan-500/50 transition-all shadow-sm hover:shadow-[0_0_15px_rgba(34,211,238,0.2)]">
                                  ...
                                  if (docItem.type === 'unverified') {
                                    return (
                                      <div key={docIdx} className="flex flex-col justify-center bg-slate-800/30 border border-slate-700/50 p-5 rounded-2xl">
                            ...
                            <div className="mt-4 border-t border-slate-700/50 pt-4">
                              <p className="text-xs text-slate-500 italic">No doctors assigned to this department yet.</p>
                            </div>
                          )}
                        </div>
                      ))}
                  ...
                  {/* Legacy Global Roster Fallback */}
                  {profile.roster?.length > 0 && (!profile.departments || profile.departments.length === 0) && (
                    <>
                      <h2 className="text-2xl font-bold text-white mb-6 font-serif border-t border-slate-700/50 pt-8">General Roster</h2>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
               ...
               <div className="mt-6 pt-6 border-t border-slate-700/50 relative z-10 flex flex-col gap-3">
                 ...
                 <div className="flex gap-2 sm:gap-3">
                   <button 
                     ...
                     className="bg-slate-800/10 hover:bg-slate-800/20 border border-slate-800/20 text-slate-700 py-3 px-4 rounded-xl flex items-center justify-center transition-colors"
                     title="Show QR Code"
                   >
                     <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm14 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z"></path></svg>
                   </button>
                   <button 
                     onClick={() => window.open(`https://api.whatsapp.com/send?text=${encodeURIComponent('Book an appointment with ' + profile.name + ' on DehaPa Hub. ' + (typeof window !== 'undefined' ? window.location.href : ''))}`, '_blank')}
      ...
      {showQRModal && (
        ...
        >
          <div 
            className="bg-white p-6 sm:p-8 rounded-[32px] shadow-2xl flex flex-col items-center max-w-sm w-full relative"
            onClick={(e) => e.stopPropagation()}
          >
            <button 
              onClick={() => setShowQRModal(false)}
              className="absolute top-4 right-4 text-slate-400 hover:text-red-500 transition-colors bg-slate-100 p-2 rounded-full"
            >
              <X className="w-5 h-5" />
            </button>
            <div className="w-16 h-16 bg-teal-50 rounded-2xl flex items-center justify-center mb-4 border border-teal-100">
              <img src={profile.image || `https://ui-avatars.com/api/?name=${encodeURIComponent(profile.name || "Provider")}&background=0f766e&color=fff`} className="w-12 h-12 rounded-xl object-cover" />
            </div>
            <h3 className="text-xl font-bold text-slate-900 text-center leading-tight mb-1">{profile.name}</h3>
            <p className="text-sm font-bold text-teal-600 mb-6">{profile.category || profile.specialty}</p>
            ...
            <p className="text-xs text-slate-500 mt-6 font-medium text-center">
              Scan this QR code with any smartphone camera to instantly open this profile.
            </p>
          </div>
        </div>
      )}
