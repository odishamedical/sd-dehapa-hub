...
if (!content.includes('id: "appointments"')) {
  content = content.replace(/id: "vault"[\s\S]*?\},/, match => newTabs + match);
}

// 2. Add hydration state and logic
const existingUseEffect = /useEffect\(\(\) => \{[\s\S]*?checkProfile\(\);\n\s*\}\n\s*\}\n\s*\}, \[router\]\);/m;
...
if (!content.includes('const [isProfileLoaded, setIsProfileLoaded] = useState(false);')) {
  ...
  useEffect(() => {
    const fetchPatientData = async () => {
      if (!userEmail) return;
      try {
        // Fetch by email if uid is unknown, or we can assume uid is in localstorage
  ...
  `;
  content = content.replace(existingUseEffect, match => match + '\n' + hydrationState);
}
...
if (!content.includes('activeTab === "appointments"')) {
  content = content.replace(/\{activeTab === "vault"[\s\S]*?\}\)/, match => newTabUIs + match);
}

fs.writeFileSync('src/app/portal/page.tsx', content);
console.log("Hydration and tabs added to portal/page.tsx");
