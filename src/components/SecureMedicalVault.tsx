          ...
          {loading ? (
            ...
            <div className="divide-y divide-slate-100/60">
              {filteredDocs.map(doc => (
                <div 
                  key={doc.id} 
                  className={`group flex items-center gap-3 md:gap-4 p-4 md:px-6 hover:bg-white hover:shadow-sm transition-all cursor-pointer ${!doc.isRead && activeFolder === 'inbox' ? 'bg-white font-bold' : 'bg-transparent text-slate-700'}`}
                ...
                >
                  ...
                  <div className="w-10 h-10 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center text-xl shrink-0 shadow-sm border border-blue-100">
                    {doc.recordType === 'prescription' ? '💊' : doc.recordType === 'lab_report' ? '🧪' : doc.recordType === 'mri' ? '🩻' : '📄'}
                  </div>
                      ...
                      <div className="flex items-center gap-2">
                        <h4 className="text-sm truncate">
                          {activeFolder === 'inbox' && doc.senderName ? (
                            <span className="text-blue-600">From: {doc.senderName}</span>
                          ) : (
                            <span className={!doc.isRead && activeFolder === 'inbox' ? 'text-slate-900' : 'text-slate-700'}>{doc.patientName}</span>
                          )}
                        </h4>
                        {!doc.isRead && activeFolder === 'inbox' && <span className="w-2 h-2 rounded-full bg-blue-600 shrink-0"></span>}
                      </div>
                      <p className={`text-xs truncate ${!doc.isRead && activeFolder === 'inbox' ? 'text-slate-700 font-semibold' : 'text-slate-500 font-medium'}`}>
                        {doc.fileName}
                      </p>
                    </div>
