...
function ClaimRedirector() {
  ...
  useEffect(() => {
    const processClaim = async () => {
      ...
      // 1. Clean the incoming phone number (just in case)
      const cleanPhone = phoneParam.replace(/\D/g, '');

      try {
        // 2. Search the directory for this phone number
        // We might need to check both exact match or if it contains the phone number
        ...
        if (!querySnapshot.empty) {
          ...
          // Redirect to the profile page with the action parameter
          router.replace(`/${urlBase}?action=claim`);
        } else {
          // If not found with exact match, try checking if they are saved with '+' or different format
          // For a production app, we'd use a cloud function to do a fuzzy search.
          // For now, if exact match fails, try adding/removing the '91' prefix
