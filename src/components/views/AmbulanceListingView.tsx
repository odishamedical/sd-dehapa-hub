...
export default function AmbulancesDirectory({ 
  ...
  const uniqueCountries = Array.from(new Set(ambulances.map((d: any) => d.country).filter(Boolean)));
  const uniqueStates = Array.from(new Set(ambulances.map((d: any) => d.state).filter(Boolean)));
  const uniqueDistricts = Array.from(new Set(ambulances.map(d => d.district).filter(d => d !== "Unknown"))).sort();
  ...
  return (
    <div className="min-h-screen bg-[#040815] text-slate-200 font-sans selection:bg-cyan-500/30">
      ...
      <div className="bg-[#0a1229] border-b border-cyan-500/20 px-6 py-3 shadow-[0_4px_20px_rgba(6,182,212,0.1)] relative z-20">
        <div className="w-full max-w-[1920px] mx-auto">
          <Breadcrumb paths={[
            ...
            ...(initialCountry ? [{ name: initialCountry.charAt(0).toUpperCase() + initialCountry.slice(1), href: `/ambulances/${initialCountry}` }] : []),
            ...(initialState ? [{ name: initialState.charAt(0).toUpperCase() + initialState.slice(1), href: `/ambulances/${initialCountry}/${initialState}` }] : []),
      ...
      <PremiumHeroSearch 
        titlePrefix="Find a"
        titleHighlight="Ambulance"
        description="Book emergency and ICU-equipped ambulances instantly."
        searchPlaceholder="e.g. 108, ALS Ambulance..."
        search={search}
        setSearch={setSearch}
      ...
      <main className="w-full max-w-[1920px] mx-auto px-6 lg:px-12 xl:px-16 py-12 relative z-10 -mt-12">
        {platformAds['ad_slot_ambulances_list_top'] && (
          <div className="w-full h-[150px] md:h-[200px] mb-8 rounded-2xl overflow-hidden border border-slate-800 shadow-sm relative bg-[#0a1229]">
            {platformAds['ad_slot_ambulances_list_top'].type === 'slider' ? (
              <AdSliderRenderer images={platformAds['ad_slot_ambulances_list_top'].sliderImages || []} linkUrl={platformAds['ad_slot_ambulances_list_top'].linkUrl} animationStyle={platformAds['ad_slot_ambulances_list_top'].animationStyle || 'fade'} />
            ) : (
              <div className="w-full h-full flex items-center justify-center" dangerouslySetInnerHTML={{ __html: platformAds['ad_slot_ambulances_list_top'].htmlCode }} />
            )}
          </div>
        )}
            ...
            <div className={`space-y-6 ${isFiltersOpen ? "block" : "hidden lg:block"}`}>
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {["ICU Equipped", "Oxygen Support", "Basic Life Support", "Hearse Van"].map(spec => (
          ...
          <div className="w-full lg:w-3/4 flex flex-col gap-6">
            <div className="grid grid-cols-1 xl:grid-cols-2 2xl:grid-cols-3 gap-6">
              {filteredAmbulances.length > 0 ? (
                filteredAmbulances.map(doc => (
                  <PremiumEntityTicket type="ambulances" key={doc.id} data={doc} />
                ))
              ) : (
                <div className="col-span-full text-center py-20 bg-[#0a1229] border border-slate-800 rounded-2xl shadow-sm">
                  ...
                  <p className="text-white font-bold text-lg mb-1">No ambulances found</p>
        ...
        {platformAds['ad_slot_ambulances_list_bottom'] && (
          <div className="w-full h-[150px] md:h-[200px] mt-12 rounded-2xl overflow-hidden border border-slate-800 shadow-sm relative bg-[#0a1229]">
            {platformAds['ad_slot_ambulances_list_bottom'].type === 'slider' ? (
              <AdSliderRenderer images={platformAds['ad_slot_ambulances_list_bottom'].sliderImages || []} linkUrl={platformAds['ad_slot_ambulances_list_bottom'].linkUrl} animationStyle={platformAds['ad_slot_ambulances_list_bottom'].animationStyle || 'fade'} />
            ) : (
              <div className="w-full h-full flex items-center justify-center" dangerouslySetInnerHTML={{ __html: platformAds['ad_slot_ambulances_list_bottom'].htmlCode }} />
            )}
          </div>
        )}
      </main>
