    ...
    const fetchLabs = async () => {
      try {
        ...
        const mappedData = docsData
          .filter((d: any) => d.category?.toLowerCase() === "lab" && d.isPublished !== false && d.adminLocked !== true)
          .map((d: any) => ({
            id: d.id,
      ...
      <main className="w-full max-w-[1920px] mx-auto px-6 lg:px-12 xl:px-16 py-12 relative z-10 -mt-12">
        {platformAds['ad_slot_labs_list_top'] && (
          <div className="w-full h-[150px] md:h-[200px] mb-8 rounded-2xl overflow-hidden border border-slate-800 shadow-sm relative bg-[#0a1229]">
            {platformAds['ad_slot_labs_list_top'].type === 'slider' ? (
              <AdSliderRenderer images={platformAds['ad_slot_labs_list_top'].sliderImages || []} linkUrl={platformAds['ad_slot_labs_list_top'].linkUrl} animationStyle={platformAds['ad_slot_labs_list_top'].animationStyle || 'fade'} />
            ) : (
              <div className="w-full h-full flex items-center justify-center" dangerouslySetInnerHTML={{ __html: platformAds['ad_slot_labs_list_top'].htmlCode }} />
            )}
          </div>
        )}
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 2xl:grid-cols-3 gap-6">
              {filteredLabs.length > 0 ? (
                filteredLabs.map(doc => (
                  <PremiumEntityTicket type="labs" key={doc.id} data={doc} />
                ))
              ) : (
                <div className="col-span-full text-center py-20 bg-[#0a1229] border border-slate-800 rounded-2xl shadow-sm">
                  ...
                  <p className="text-white font-bold text-lg mb-1">No labs found</p>
        ...
        {platformAds['ad_slot_labs_list_bottom'] && (
          <div className="w-full h-[150px] md:h-[200px] mt-12 rounded-2xl overflow-hidden border border-slate-800 shadow-sm relative bg-[#0a1229]">
            {platformAds['ad_slot_labs_list_bottom'].type === 'slider' ? (
              <AdSliderRenderer images={platformAds['ad_slot_labs_list_bottom'].sliderImages || []} linkUrl={platformAds['ad_slot_labs_list_bottom'].linkUrl} animationStyle={platformAds['ad_slot_labs_list_bottom'].animationStyle || 'fade'} />
            ) : (
              <div className="w-full h-full flex items-center justify-center" dangerouslySetInnerHTML={{ __html: platformAds['ad_slot_labs_list_bottom'].htmlCode }} />
            )}
          </div>
        )}
      </main>
