...
interface EndorsementProps {
  ...
  targetProviderRole?: string;
  currentUserId: string | null;
  currentUserRole: string;
}

export default function ProviderEndorsementWidget({ targetProviderId, targetProviderRole, currentUserId, currentUserRole }: EndorsementProps) {
  const [endorsements, setEndorsements] = useState<any[]>([]);
  const [hasEndorsed, setHasEndorsed] = useState(false);
  const [loading, setLoading] = useState(true);

  const isSelf = currentUserId === targetProviderId;
  const canEndorse = !isSelf && currentUserRole !== 'patient' && currentUserRole !== 'user';
  ...
  useEffect(() => {
    ...
    fetchEndorsements();
  }, [targetProviderId, currentUserId]);
  ...
  const toggleEndorsement = async () => {
    ...
    const endorserName = localStorage.getItem("sd_current_user_name") || "Verified Provider";
    
    const endorsementId = `${currentUserId}_${targetProviderId}`;
        ...
        >
          {hasEndorsed ? (
            ...
            <>
              ...
              Endorse {targetProviderRole ? targetProviderRole.charAt(0).toUpperCase() + targetProviderRole.slice(1) : 'Provider'}
            </>
          )}
        </button>
      )}
          ...
          endorsements.map((end, idx) => (
            <div key={idx} className="bg-slate-800/50 rounded-xl p-3 flex items-center gap-3 border border-slate-700/50">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-[10px] text-slate-400">Verified Provider</p>
              </div>
            </div>
          ))
        )}
