...
export default function AmbulanceProfileView({ id, customSlug }: { id?: string, customSlug?: string }) {
  ...
  const handleInlineSave = async (field: string, value: any) => {
    ...
    try {
      const docRef = doc(db, 'directory', ambulance.id);
      await updateDoc(docRef, { [field]: value });
      setAmbulance((prev: any) => ({ ...prev, [field]: value }));
    } catch (err) {
      ...
      <div className="w-full max-w-[1920px] mx-auto px-6 lg:px-12 xl:px-16 relative -mt-24 z-20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 items-start">
          ...
          <div className="lg:col-span-3 space-y-8">
            ...
            <TicketCard 
              entity={ambulance} 
              config={TicketConfig.ambulance} 
              isEditMode={isEditMode}
              onSave={handleInlineSave}
            />

            {/* Google Extracted Image Gallery */}
            {ambulance.galleryImages?.length > 0 && (
              <div className="bg-white rounded-2xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {ambulance.galleryImages.map((img: string, idx: number) => (
                    <div key={idx} className="aspect-square rounded-xl overflow-hidden shadow-sm border border-slate-200">
                      <img src={img} alt={`Ambulance Photo ${idx + 1}`} className="w-full h-full object-cover hover:scale-110 transition-transform duration-500" />
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-8">
                ...
                <div className="bg-white rounded-2xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 relative group">
                  ...
                  <InlineEditArray 
                    items={ambulance.specialties || []} 
                    onSave={(newArr) => handleInlineSave('specialties', newArr)} 
                    isEditMode={isEditMode} 
                    placeholder="Add a service (e.g. Oxygen Support, ICU)"
                  />
                </div>
                    ...
                    <div className="space-y-4 mt-4 relative">
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="text-sm text-slate-600 leading-relaxed">
                          <InlineEditField 
                            value={ambulance.clinic.address} 
