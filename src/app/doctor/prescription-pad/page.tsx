          ...
          <form id="rx-form" onSubmit={handleSaveAndSend} className="p-8 space-y-8">
            ...
            <GlobalFormCard className="space-y-6">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 pb-4">
                ...
                <div className="flex flex-col md:flex-row items-start md:items-center gap-2 md:gap-3">
                  <GlobalLabel className="mb-0 text-[10px]">Send to Pharmacy:</GlobalLabel>
                  <GlobalSelect 
                    value={rxData.routing.pharmacyId}
                    onChange={e => setRxData(prev => ({...prev, routing: {...prev.routing, pharmacyId: e.target.value}}))}
                    className="border-teal-200 text-teal-800 !py-2 !px-3 !rounded-lg"
                  >
              ...
              <div className="space-y-4">
                {rxData.medicines.map((med, index) => (
                  <div key={index} className="flex flex-col md:flex-row gap-4 items-start bg-white p-5 rounded-2xl border border-slate-200 shadow-sm relative">
                    <div className="absolute top-0 right-0 p-2">
                      <span className="text-xs font-bold text-slate-300 bg-slate-50 px-2 rounded-full border border-slate-100">#{index + 1}</span>
                    </div>
                    <div className="flex-1 w-full pt-1">
                      <GlobalLabel>Medicine Name</GlobalLabel>
                      <datalist id="favorite-medicines">
                        {allMedicinesList.map((m, i) => <option key={i} value={m} />)}
                      </datalist>
                      <GlobalInput list="favorite-medicines" type="text" placeholder="e.g. Paracetamol 650mg" value={med.name} onChange={e => handleMedicineChange(index, "name", e.target.value)} required />
                    </div>
                    <div className="w-full md:w-32 pt-1">
                      <GlobalLabel>Dosage</GlobalLabel>
                      <GlobalInput type="text" placeholder="e.g. 1 tab" value={med.dosage} onChange={e => handleMedicineChange(index, "dosage", e.target.value)} required />
                    </div>
                    <div className="w-full md:w-48 pt-1">
                      <GlobalLabel>Frequency</GlobalLabel>
                      <GlobalSelect value={med.frequency} onChange={e => handleMedicineChange(index, "frequency", e.target.value)} required>
                        <option value="">Select...</option>
                    ...
                    <div className="w-full md:w-32 pt-1">
                      <GlobalLabel>Duration</GlobalLabel>
                      <GlobalInput type="text" placeholder="e.g. 5 days" value={med.duration} onChange={e => handleMedicineChange(index, "duration", e.target.value)} required />
                    </div>
                  </div>
                ))}
              </div>
            ...
            <GlobalFormCard className="space-y-6">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 pb-4">
                ...
                <div className="flex flex-col md:flex-row items-start md:items-center gap-2 md:gap-3">
                  <GlobalLabel className="mb-0 text-[10px]">Send to Lab:</GlobalLabel>
                  <GlobalSelect 
                    value={rxData.routing.labId}
                    onChange={e => setRxData(prev => ({...prev, routing: {...prev.routing, labId: e.target.value}}))}
                    className="border-blue-200 text-blue-800 !py-2 !px-3 !rounded-lg"
                  >
                    <option value="">Do Not Send (Give to Patient)</option>
                ...
                {rxData.tests.map((test, index) => (
                  <div key={index} className="flex flex-col md:flex-row gap-4 items-start bg-white p-5 rounded-2xl border border-slate-200 shadow-sm relative">
                    ...
                    <div className="w-full md:w-1/2 pt-1">
                      <GlobalLabel>Test Name</GlobalLabel>
                      <GlobalInput type="text" placeholder="e.g. CBC, Lipid Profile" value={test.name} onChange={e => handleTestChange(index, "name", e.target.value)} required />
                    </div>
                    <div className="flex-1 w-full pt-1">
                      <GlobalLabel>Instructions</GlobalLabel>
                      <GlobalInput type="text" placeholder="e.g. Fasting" value={test.instructions} onChange={e => handleTestChange(index, "instructions", e.target.value)} />
                    </div>
                  </div>
                ))}
              </div>
            ...
            {/* General Advice */}
            <div className="space-y-2">
              <GlobalLabel>General Advice & Follow Up</GlobalLabel>
              <GlobalTextarea 
                value={rxData.advice}
